...
from tensorflow.keras import  datasets, optimizers, layers, regularizers
...
class DAoG(optimizers.Optimizer):
  ...
  def _resource_apply_dense(self, grad, var):
      ...
      et = tf.pow(norm_x, 1 / (4 )) / tf.pow(norm_grad, 1 / (5 - 0.06 * local_step / 100))
...
class LearningRateEpochLogger(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if m is not None and wm is not None:
            ...
            if hasattr(lr, "numpy"):
                lr = lr.numpy()
            effective_lr = lr * np.power(m_val, 1/4 ) / np.power(wm_val, 1 / (5- 0.06 * local_step / 100))  #DAoG
...
history = {name: {'train_loss': [], 'train_accuracy': [], 'test_loss': [], 'test_accuracy': []} for name in
           optimizers.keys()}
...
for opt_name, optimizer in optimizers.items():
    ...
    def build_dpcnn():
        ...
        x = layers.Embedding(input_dim=vocab_size, output_dim=128, input_length=max_length)(inputs)

        x = layers.Conv1D(filters=64, kernel_size=3, padding='same', activation='relu', kernel_regularizer=regularizers.l2(1e-4))(x)
        ...
        def conv_block(x):
            ...
            x = layers.Conv1D(filters=64, kernel_size=3, padding='same', kernel_regularizer=regularizers.l2(1e-4))(x)
