...
# Download the WikiBooks pre-trained BERT model from the following Kaggle link:
# https://www.kaggle.com/models/google/experts-bert/tensorFlow2/wiki-books
bert_path = "D:/models/wikibert"


preprocessor = hub.KerasLayer(preprocessor_path)
bert_encoder = hub.KerasLayer(bert_path, trainable=False)
...
train_sentences, val_sentences, train_labels, val_labels = (
    sampled_train_data["sentence"],
    test_data["sentence"],
    sampled_train_data["label"],
    test_data["label"]
)

train_labels = pd.to_numeric(train_labels, errors='coerce').astype(np.float32)
val_labels = pd.to_numeric(val_labels, errors='coerce').astype(np.float32)

train_sentences = train_sentences.astype(str)
val_sentences = val_sentences.astype(str)

def build_model(optimizer):
    inputs = tf.keras.layers.Input(shape=(), dtype=tf.string, name="text")

    preprocessed_text = preprocessor(inputs)
...
class DAoG(optimizers.Optimizer):
  ...
  def _resource_apply_dense(self, grad, var):
      ...
      et = tf.pow(norm_x, 1 / (4 )) / tf.pow(norm_grad, 1 / (5 - 0.06 * local_step / 53))
...
class LearningRateEpochLogger(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if m is not None and wm is not None:
            ...
            if hasattr(lr, "numpy"):
                lr = lr.numpy()
            effective_lr = lr * np.power(m_val, 1/4 ) / np.power(wm_val, 1 / (5- 0.06 * local_step / 53))  #DAoG
